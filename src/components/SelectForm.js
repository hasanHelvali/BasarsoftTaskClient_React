...
function SelectForm({ onSelectChange }) {
    ...
    const { featureType,handleFeatureType} = useMyContext();
    useEffect(() => {
    ...
    const handleSelectChange = (event) => {
        ...
        handleFeatureType(event.target.value)
    ...
    <>
        <form className="form">
            ...
            <select id="type" className="form-select" onChange={handleSelectChange}  value={featureType}>
                {/* selectbox degistiginde handleSelectChange adında bir fonksiyonu tetikliyorum. */}
